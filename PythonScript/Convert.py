...
def Convert(content):
    for key in charsAfterOpenCcConversion_To_CharsShownInSimplifiedText:
        content = content.replace(
            key, charsAfterOpenCcConversion_To_CharsShownInSimplifiedText[key])
...
def wrap_checkedToneChar_with_superscript(content):
    ...
    for c in content:
        result += superscriptSignInPandoc + c + \
            superscriptSignInPandoc if c in checkedToneChars else c
    return result
